...
# [필독] 본인의 웹 앱 URL을 여기에 붙여넣으세요
API_URL = "여기에_복사한_URL을_넣으세요"
...
# 한국 시간 기준으로 오늘 날짜 설정
now = datetime.now()
selected_date = st.date_input("📅 운행 날짜", now)
...
if st.button("🚀 운행 기록 저장", use_container_width=True, type="primary"):
    if end_km < start_km:
        ...
        payload = {
            ...
            "운행내용": purpose,
            "비고": memo,
            "입력시간": datetime.now().strftime('%Y-%m-%d %H:%M:%S') # 현재 시간 전송
        }
        
        try:
            response = requests.post(API_URL, data=json.dumps(payload))
            if response.text == "Success":
                st.success("✅ 한국 시간으로 저장되었습니다!")
                st.balloons()
            else: st.error("저장 실패")
