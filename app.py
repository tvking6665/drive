...
st.markdown("""
    ...
    .stCaption { font-size: 13px !important; color: #FF4B4B !important; margin-top: -5px !important; margin-bottom: 10px !important; }
...
with col_s:
    ...
    # 친절각형 문구 반영
    st.caption(f"💡 마지막 기록된 거리입니다: {last_km}km")

with col_e:
    dest_list = ["구산", "KDS", "고광메탈", "한국하이테크", "송원테크", "제이테크", "DLS성주", "AST", "평화정공", "왜관(VPHC)", "동아금속", "통근노선 종점", "직접 입력"]
...
with col_purpose:
    purpose = st.selectbox("📝 운행 내용", ["납품", "업무협의", "통근버스 운행", "거래처 미팅", "현장 방문", "기타"])
with col_memo:
...
if st.button("🚀 운행 기록 저장", use_container_width=True, type="primary"):
    if end_km < start_km:
        ...
        payload = {
            "날짜": selected_date.strftime('%Y-%m-%d'), "차량": selected_car, "운전자": selected_driver,
            "출발지": start_node, "목적지": end_node, "시작거리": start_km, "종료거리": end_km,
            "주행거리": end_km - start_km, "운행내용": purpose, "비고": memo, "입력시간": current_kst.strftime('%Y-%m-%d %H:%M:%S')
        }
        try:
...
try:
    ...
    if not df_history.empty:
        filtered_df = df_history if view_car == "전체보기" else df_history[df_history['차량'] == view_car]
        display_df = filtered_df.tail(10).iloc[::-1]
        st.dataframe(display_df, use_container_width=True, hide_index=True)
    else: st.write("등록된 기록이 없습니다.")
except: st.write("데이터 로딩 중...")
