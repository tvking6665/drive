...
view_car = st.selectbox("조회할 차량을 선택하세요", ["전체보기"] + car_list)
    ...
    try:
        ...
        if not df_history.empty:
            ...
            if view_car != "전체보기":
                ...
                title_text = f"🔎 {view_car} 최근 기록 (최대 10건)"
            else:
                filtered_df = df_history
                title_text = "🔎 전체 차량 최근 기록 (최대 10건)"
            ...
            if not display_df.empty:
                ...
                st.info(f"'{view_car}'의 운행 기록이 없습니다.")
        else:
