...
# 3. 입력 섹션
# 한국 시간(KST) 강제 설정 (서버가 어디든 +9시간 적용)
...
if st.button("🚀 운행 기록 저장", use_container_width=True, type="primary"):
    if end_km < start_km:
        ...
        payload = {
            ...
            "운행내용": purpose,
            "비고": memo,
            "입력시간": current_kst.strftime('%Y-%m-%d %H:%M:%S') # 한국 시간 전송
        }
        
        try:
            response = requests.post(API_URL, data=json.dumps(payload))
            if response.text == "Success":
                st.success("✅ 한국 시간으로 정확히 저장되었습니다!")
                st.balloons()
            else: st.error("저장 실패")
